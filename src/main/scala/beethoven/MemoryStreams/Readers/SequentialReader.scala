...
class SequentialReader(
  ...
  val largeTxNBeats =
    Math.max(platform.prefetchSourceMultiplicity, userBytes / fabricBeatBytes)
  ...
  val rowsAvailableToAlloc = RegInit(
    prefetchDepthRows.U(log2Up(prefetchDepthRows + 1).W)
  )
  require(
    isPow2(userBytes),
    "The width of the reader channel must be a power of two"
  )
  ...
  val prefetch_readIdx, prefetch_writeIdx = RegInit(
    0.U(log2Up(prefetchDepthRows).W)
  )
  ...
  val hasDualPortMemory = platform.platformType match {
    ...
    case PlatformType.ASIC =>
      platform match {
        case compiler: HasMemoryCompiler =>
          useRegMem || compiler.memoryCompiler.mostPortsSupported >= 2
  ...
  if (!hasDualPortMemory && !SequentialReader.has_warned_dp) {
    ...
    System.err.println(
      "Warning: CReader is using a single port memory. This may cause performance degradation."
    )
  }
  val prefetch_buffers_valid = RegInit(
    VecInit(Seq.fill(prefetchDepthRows)(false.B))
  )
  ...
  val (write_ready, read_ready, write_port_idx, read_port_idx) =
    if (hasDualPortMemory) {
      ...
      val (ridx, widx) = if (prefetch_buffers.nReadPorts == 0) {
        ...
        (
          prefetch_buffers.getReadPortIdx(0),
          prefetch_buffers.getWritePortIdx(0)
        )
      }
  ...
  val (slots_per_alloc, small_tx_mult) = if (fabricBeatBytes >= userBytes) {
    ...
    val beatBufferPerSource = Reg(
      Vec(nSources, Vec(beatsPerLine - 1, UInt(fabricBeatBits.W)))
    )
    val perSourceBeatProgress = Reg(Vec(nSources, UInt(log2Up(beatsPerLine).W)))
    when(tl_reg.io.enq.fire) {
      assert(
        tl_reg.io.enq.bits.size >= log2Up(fabricBeatBytes).U,
    ...
    when(tl_out.d.fire) {
      ...
      when(perSourceBeatProgress(src) === (beatsPerLine - 1).U) {
        ...
        prefetch_buffers.data_in(write_port_idx) := Cat(
          tl_out.d.bits.data,
          Cat(beatBufferPerSource(src).reverse)
        )
        prefetch_buffers.data_in(write_port_idx) := Cat(
          tl_out.d.bits.data,
          Cat(beatBufferPerSource(src).reverse)
    ...
    require(
      isPow2(beatsPerLine),
      s"the reader width must be a multible of ${fabricBeatBytes}."
    )
    ...
    is(s_send_mem_request) {
      when(len < largestRead.U) {
        ...
        when(tl_reg.io.enq.fire) {
          ...
          prefetch_writeIdx := Mux(
            prefetch_writeIdx === (prefetchDepthRows - slots_per_alloc).U,
            0.U,
            prefetch_writeIdx + slots_per_alloc.U
          )
  ...
  val channel_buffer_q = Module(
    new Queue(UInt(storedDataWidth.W), channel_buffer_depth)
  )
  ...
  when(
    prefetch_buffers_valid(
      prefetch_readIdx
    ) && reads_in_flight + queue_occupancy < channel_buffer_depth.U && read_ready
  ) {
  ...
  when(io.channel.data.fire) {
    ...
    when(
      data_channel_read_idx === (fabricBeatsPerBufferRow - 1).U || expectedChannelBeats === 1.U
    ) {
      data_channel_read_idx := 0.U
