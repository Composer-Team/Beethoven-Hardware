...
class ScratchpadDataPort(val scReqBits: Int, val dataWidthBits: Int)
    extends ScratchpadPort {
...
/** Parameters that all scratchpad subtypes should support
  *
  * @param dataWidthBits
  *   the granularity of a single scratchpad read/write in bits
  * @param nDatas
  *   number of data items in the scratchpad. Non-zero
  * @param latency
  *   latency of a scratchpad access from the user interface. Current
  *   implementation only supports 1 or 2.
  * @param specialization
  *   How data is loaded from memory. Choose a specialization from
  *   ScratchpadSpecialization
  */
class MemoryScratchpad(csp: ScratchpadConfig)(implicit p: Parameters)
    extends LazyModule {
  require(
    csp.dataWidthBits.intValue() > 0,
    "The Scratchpad datawidth must be greater than 0."
  )
  require(
    csp.nDatas.intValue() > 0,
    "The scratchpad depth (number of rows) must be greater than 0."
  )
  ...
  if (useLowResourceReader && !has_warned) {
    has_warned = true
    System.err.println(
      f"The ${csp.name} scratchpad is using a _very_ wide data bus, preventing use of the high" +
        f" throughput reader modules. To use these, either increase platform prefetch length to ${csp.dataWidthBits
            .intValue() / channelWidthBytes / 8}" +
        f" or decrease bus width to at most ${channelWidthBytes * 8 * platform.prefetchSourceMultiplicity}."
    )
  }
  val very_small_sp = (csp.dataWidthBits.intValue() * csp.nDatas
    .intValue()) <= (channelWidthBytes * 8 * platform.prefetchSourceMultiplicity)
          ...
          TLMasterPortParameters.v2(
            masters = Seq(
              TLMasterParameters.v1(
                ...
                sourceId = IdRange(
                  0,
                  if (useLowResourceReader || very_small_sp) 1
                  else platform.defaultReadTXConcurrency
                ),
                supportsProbe = TransferSizes(
                  channelWidthBytes,
                  channelWidthBytes * platform.prefetchSourceMultiplicity
                ),
          ...
          TLMasterPortParameters.v2(
            masters = Seq(
              TLMasterParameters.v1(
                ...
                sourceId = IdRange(
                  0,
                  if (!very_small_sp) platform.defaultWriteTXConcurrency else 1
                ),
                supportsProbe = TransferSizes(
                  channelWidthBytes,
                  channelWidthBytes * platform.prefetchSourceMultiplicity
...
class ScratchpadImpl(csp: ScratchpadConfig, outer: MemoryScratchpad)
  ...
  private val realNRows = Math.max(
    (nDatas.toFloat / datasPerCacheLine).ceil.toInt,
  ...
  private val memory = Seq.fill(datasPerCacheLine)(
    Memory(
      latency,
      dataWidth = dWidth,
      nRows = realNRows,
  ...
  if (csp.features.readOnly) {
    IOs.foreach { io =>
      assert(
        !io.req.valid || !io.req.bits.write_enable,
  ...
  require(
    isPow2(datasPerCacheLine),
    "The Scratchpad nBanks/datas-per-cache-line parameter must be a power of 2."
  )
  if (outer.mem_reader.isDefined) {
    ...
    val loader = Module(specialization match {
      case psw: PackedSubwordScratchpadConfig =>
        require(
          datasPerCacheLine == 1,
          "Using packed subword, only assumes nbanks=1 in the Scratchpad"
        )
      ...
      val reader = if (outer.useLowResourceReader) {
        ...
        val reader = Module(
          new LightweightReader_small(
            ...
            tl_edge = outer.mem_reader.get.out(0)._2,
            sp_sz_bytes =
              csp.nDatas.intValue() * csp.dataWidthBits.intValue() / 8
          )
        )
        reader.tl_out <> outer.mem_reader.get.out(0)._1
        linearReadInc()
        ...
        val expectedBeatsLeft = RegInit(
          0.U((tl_out.params.addressBits - beatBytesWidth).W)
        )

        val bigTxBytes = platform.prefetchSourceMultiplicity * beatBytes

        val sourceIdle = RegInit(
          VecInit(Seq.fill(tl_edge.client.endSourceId)(true.B))
        )
        val hasIdle = sourceIdle.asUInt =/= 0.U
        val nextSource = PriorityEncoder(sourceIdle)

        val offsetAcc = Reg(UInt(log2Up(csp.nDatas).W))
        val spadAddrOffsetPerSource = Reg(
          Vec(tl_edge.client.endSourceId, UInt(log2Up(csp.nDatas).W))
        )
        val beatsLeftPerSourcee = Reg(
          Vec(
            tl_edge.client.endSourceId,
        ...
        when(tl_out.d.fire) {
          ...
          spadAddrOffsetPerSource(src) := spadAddrOffsetPerSource(
            src
          ) + loader.spEntriesPerBeat.U
        }
        ...
        when(state === s_idle) {
          ...
          when(hasIdle && expectedBeatsLeft =/= 0.U) {
            val isBig =
              expectedBeatsLeft >= platform.prefetchSourceMultiplicity.U
            ...
            when(tl_out.a.fire) {
              ...
              beatsLeftPerSourcee(nextSource) := Mux(
                isBig,
                (platform.prefetchSourceMultiplicity - 1).U,
                0.U
              )

              addr := addr + Mux(
                isBig,
                platform.prefetchSourceMultiplicity.U,
                1.U
              )
              offsetAcc := offsetAcc + Mux(
                isBig,
                (loader.spEntriesPerBeat * platform.prefetchSourceMultiplicity).U,
                loader.spEntriesPerBeat.U
              )
              expectedBeatsLeft := expectedBeatsLeft - Mux(
                isBig,
                platform.prefetchSourceMultiplicity.U,
                1.U
              )
          ...
          when(
            loader.io.cache_block_in.ready && sourceIdle.asUInt === BigInt(
              "1" * tl_edge.client.endSourceId,
              radix = 2
            ).U
          ) {
            state := s_idle
    ...
    if (supportWriteback) {
      require(
        specialization
          .isInstanceOf[FlatPackScratchpadConfig] && dWidth % 8 == 0,
      ...
      val mem_valid = ShiftReg(
        memory(0).read_enable(0) && wb_state === wb_read,
        latency,
        clock
      )
      ...
      req.writeback.ready := false.B
      assert(
        req.writeback.valid === false.B || reset.asBool === true.B,
