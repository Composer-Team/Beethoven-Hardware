...
class MemWritePort(
    ...
    val canSelectChannel: Boolean
)(implicit p: Parameters)
    extends DecoupledIO(new Bundle() {
      val data = UInt(dataBits.W)
      val addr = UInt(addrBits.W)
      val core =
        if (canSelectCore) Some(UInt(beethoven.Systems.getCommMemCoreBits().W))
        else None
      val channel =
        if (canSelectChannel)
          Some(UInt(beethoven.Systems.getCommMemChannelBits().W))
        else None
    })

class IntraCoreScratchpad(
    ...
    extends LazyModule {
  require(
    dataWidthBits.intValue() > 0,
    "The scratchpad must have a data-width greater than 0."
  )
  require(
    nDatas.intValue() > 0,
    "The scratchpad must have a number of rows greater than 0"
  )
    ...
    TLSlavePortParameters.v1(
      managers = Seq(
        TLSlaveParameters.v1(
          address = Seq(
            getCommMemAddressSet(systemParams.name, coreIdx, mp, channel_id)
          ),
...
class IntraCoreScratchpadImp(
    dataWidthBits: Int,
    nDatas: Int,
    latency: Int,
    nPorts: Int,
    readOnly: Boolean,
    outer: IntraCoreScratchpad
) extends LazyModuleImp(outer) {
  private val scReqBits = log2Up(nDatas)
  val IOs =
    Seq.fill(nPorts)(IO(new ScratchpadDataPort(scReqBits, dataWidthBits)))
  ...
  if (readOnly) {
    IOs.foreach { io =>
      assert(
        !io.req.valid || (io.req.valid && !io.req.bits.write_enable),
        "read only scratchpad. turn off readonly if you need to write to the scratchpad"
      )
    }
  }
