...
class BRAMDualClocked(minDepth: Int, minWidth: Int) extends BlackBox {
  ...
  val realWidth = validWidths.filter(_ >= minWidth).minOption.getOrElse {
    throw new Exception(
      f"Requested width for FIFO ($minWidth) - unsatisfiable. Must be <= 72"
    )
  }
  val realDepth =
    widthToValidDepths(realWidth).filter(_ >= minDepth).minOption.getOrElse {
      throw new Exception(
        f"Requested depth for FIFO ($minDepth) - unsatisfiable." +
