...
class AWSF1Platform(memoryNChannels: Int, val clock_recipe: String = "A0")
  ...
  override val clockRateMHz: Int = clock_recipe match {
    ...
    case _ =>
      throw new Exception(
        "Invalid clock recipe. Only supporting A0, A1, A2 for main clock right now."
      )
  }
  ...
  override def postProcessorMacro(c: Parameters, paths: Seq[Path]): Unit = {
    if (c(BuildModeKey) == BuildMode.Synthesis) {
      ...
      println(
        "Enter the AWS F1 instance EC2 instance IP address (blank if store locally) :"
      )
        ...
        while (fail) {
          ...
          try {
            ...
            os.proc(
              ...
              "rm",
              "-rf",
              "~/build-dir/generated-src/*"
            )
            os.proc(
              "rsync",
              "-avz",
              f"$gen_dir/",
              f"ec2-user@$in:~/build-dir/generated-src/"
            ).call(
              stdout = os.Inherit
            )
            os.proc(
  ...
  ) ++ (if (memoryNChannels <= 1) List()
        else
          (1 until memoryNChannels).map(a => PhysicalMemoryInterface(a - 1, a)))
  ...
  override def placementAffinity: Map[Int, Double] =
    Map.from(Seq((0, 1.0), (1, 1.0), (2, 1.7)))
