...
class AXI_ClockFIFO(addrWidth: Int, dataWidth: Int, idWidth: Int, synchronization_stages: Int = 3) extends BlackBox {
  ...
  val io = IO(new Bundle {
    ...
    val m_axi = new AXI4Compat(MasterPortParams(0, 1L << addrWidth, dataWidth / 8, idWidth, dataWidth / 8 * 64))

    val s_axi_aclk = Input(Bool())
    val s_axi_aresetn = Input(Bool())
    val s_axi = Flipped(new AXI4Compat(MasterPortParams(0, 1L << addrWidth, dataWidth / 8, idWidth, dataWidth / 8 * 64)))
  })
