...
object LazyModuleWithSLRs {
  ...
  private[beethoven] var toplevelObjectsPerSLR: List[(Int, LazyModule)] =
    List.empty
  private[beethoven] def clearObjectsPerSLR(): Unit = toplevelObjectsPerSLR =
    List.empty
  ...
  def LazyModuleWithFloorplan[T <: LazyModule](
      mod: T,
      slr_id: Int,
      name: String
  ): T = {
