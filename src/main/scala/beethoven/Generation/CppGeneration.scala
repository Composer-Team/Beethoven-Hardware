...
object CppGeneration {
  ...
  private[beethoven] case class CppDefinition(
      ty: String,
      name: String,
      value: String
  )

  private[beethoven] case class PreprocessorDefinition(
      ty: String,
      value: String
  )

  private[beethoven] case class HookDef(
      sysName: String,
      cc: AccelCommand,
      resp: AccelResponse,
      opCode: Int
    ...
    cc.elements.foreach { p =>
      ...
      val paramName = p._1
      assert(
        data.getWidth <= 64 || data.getWidth % 8 == 0,
  ...
  def addUserCppDefinition[t](
      ty: String,
      name: String,
      value: t,
      resolveConflicts: Option[(String, t) => t]
  ): Unit = {
    val ty_f = ty.trim
    val name_f = name.trim
    val existingDefs = user_defs.filter(_.name == name_f)
    if (existingDefs.isEmpty)
      user_defs = CppDefinition(ty_f, name_f, value.toString) :: user_defs
    else {
      resolveConflicts match {
        case Some(resolver) =>
          val without = user_defs.filter(_.name != name_f)
          user_defs = CppDefinition(
            ty_f,
            name_f,
            resolver(existingDefs(0).value, value).toString
          ) :: without
        case None =>
          existingDefs.foreach(a =>
            require(
              a.ty == ty_f && a.value == value.toString,
              s"Redefining ${a.name} from (${a.ty}, ${a.value}) to ($ty, $value)"
            )
          )
      }
    }
  ...
  private[beethoven] def addUserCppFunctionDefinition(
      systemName: String,
      cc: AccelCommand,
      resp: AccelResponse,
      opCode: Int
  ...
  def addUserCppDefinition[t](
      elems: Seq[(String, String, t)],
      resolver: Option[(String, t) => t] = None
  ): Unit = {
