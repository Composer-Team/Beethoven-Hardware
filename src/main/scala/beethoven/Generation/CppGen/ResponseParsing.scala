...
import beethoven.{
  AccelResponse,
  AccelRoccUserResponse,
  EmptyAccelResponse,
  InvalidAccelResponse
}

object ResponseParsing {
  private[beethoven] case class BeethovenResponseDeclarations(
      name: String,
      dec: String,
      definition: String
  )
  ...
  def getResponseDeclaration(
      resp: AccelResponse,
      sysName: String
  ): Option[BeethovenResponseDeclarations] = {
    resp match {
      case _: EmptyAccelResponse =>
        return Some(
          BeethovenResponseDeclarations(
            "bool",
            f"""
      ...
      case _ =>
        if (bundlesAreEquivalentEnough(resp, new AccelRoccUserResponse()))
          return Some(
            BeethovenResponseDeclarations("beethoven::rocc_response", "", "")
          )
    }

    // otherwise we have a custom type
    val structName = f"$sysName::${resp.responseName}"
    val structMembersWithType = resp.realElements.map(a =>
      f"${getCType(resp.elements(a._1), a._1)} ${a._1}"
    )
    val response_struct =
      f"""
         |namespace $sysName {
         ...
         |      ${safe_join(
          resp.realElements.map(a => f"${a._1}(${a._1})"),
          ", "
        )}
         |      {}
    ...
    val template_sig =
      f"template<> $structName beethoven::response_handle<$structName>::get()"
    val try_template_sig =
      f"template<> std::optional<$structName> beethoven::response_handle<$structName>::try_get()"
    val dec =
      f"""
         |$response_struct;
         |$template_sig;
    ...
    val template_decs =
      resp.fieldSubranges.filter(_._1.endsWith("_FP")).map { ele =>
    ...
    val template_def = resp.fieldSubranges
      .map { ele =>
        if (ele._1.endsWith("_FP")) {
          ...
          (
            ele._1,
            f"(resp & (0x${mask.toHexString}LL << ${ele._2._2})) >> ${ele._2._2}"
          )
        }
      }
      .sortBy(_._1)
