...
class ClockCrossingImp(outer: ClockCrossing)(implicit p: Parameters)
  ...
  if (platform.platformType != PlatformType.FPGA) {
    System.err.println(
      "Clock crossing implementation currently relies on dual-clocked BRAM (FPGA-only)," +
        " so other platforms are not yet supported."
    )
    System.exit(0)
  }

  outer.node.out.zip(outer.node.in).foreach {
    case ((bundle_o, param_o), (bundle_i, param_i)) =>
      // maintain a separate FIFO for each channel because they are completely independent by specification
      val cross = Module(
        new AXI_ClockFIFO(
          param_i.bundle.addrBits,
          param_i.bundle.dataBits,
          param_i.bundle.idBits
        )
      )
