...
class AXIFrontBusProtocol(withDMA: Boolean, nClocks: Int = 1) extends FrontBusProtocol {
  override def deriveTopIOs(config: Parameters)(implicit p: Parameters): Parameters = {
    val (port_cast, dma_cast, mem_front) = config(OptionalPassKey).asInstanceOf[(AXI4MasterNode, Option[AXI4MasterNode], Seq[AXI4SlaveNode])]
    ...
    val mems = {
      ...
      (M00_AXI zip ins) foreach { case (i, (o, _)) =>
        AXI4Compat.connectCompatMaster(i, o, if (platform.hasDebugAXICACHEPROT) config(DebugCacheProtSignalKey) else None)
      }
    ...
    config.alterPartial({
      ...
      case ResetKey => if (platform.isActiveHighReset) resets else resets.map(!_)
    })
  }
