...
class TLSlaveBufferedBroadcast(depth: Int = 2)(implicit p: Parameters)
  ...
  val module = new LazyModuleImp(this) {
    ...
    node.out.foreach { case (out, _) =>
      out.tl.valid := ShiftReg(
        in.tl.valid,
        depth,
        clock,
        allow_fpga_shreg = false,
        as = _.asBool
      )
      out.tl.bits := ShiftReg(
        in.tl.bits,
        depth,
        clock,
        allow_fpga_shreg = false,
...
object TLSlaveBufferedBroadcast {
  ...
  def apply(depth: Int = 2)(implicit p: Parameters): TLSlaveBroadcastNode = {
    LazyModuleWithFloorplan(
      new TLSlaveBufferedBroadcast(depth), {
  ...
  def apply(name: String, depth: Int)(implicit
      p: Parameters
  ): TLSlaveBroadcastNode = {
