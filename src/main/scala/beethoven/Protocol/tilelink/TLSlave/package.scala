...
package object TLSlave {
  ...
  object TLSlaveOnlyImp
    ...
    override def render(e: TLEdgeOut): RenderedEdge =
      RenderedEdge(colour = "#00ff00")
  }

  type TLSlaveNode = NodeHandle[
    TLMasterPortParameters,
    TLSlavePortParameters,
    TLEdgeOut,
    TLSlaveOnlyBundle,
  ...
  case class TLSlaveManagerNode(TLSlavePortParameters: TLSlavePortParameters)(
      implicit
      valName: ValName,
      p: Parameters
  ) extends SinkNode(TLSlaveOnlyImp)(pi = Seq(TLSlavePortParameters))

  case class TLSlaveBroadcastNode()(implicit valName: ValName)
      extends NexusNode(TLSlaveOnlyImp)(
        dFn = { mps =>
          require(mps.length == 1, "Only one source node allowed"); mps.head
        },
        uFn = { sp =>
          // ensure that all of the managers are the same
          val head_slave_set = sp.head.slaves.flatMap(_.address)

          def addr_eq(as1: AddressSet, as2: AddressSet): Boolean =
            as1.mask == as2.mask && as1.base == as2.base
  ...
  case class TLToTLSlaveNode()(implicit valName: ValName)
      extends MixedAdapterNode(TLImp, TLSlaveOnlyImp)(
        dFn = { mp =>
  ...
  case class TLSlaveIdentityNode()(implicit valName: ValName)
      extends IdentityNode(TLSlaveOnlyImp)()

  case class TLSlaveNexusNode(
      dFn: Seq[TLMasterPortParameters] => TLMasterPortParameters,
      uFn: Seq[TLSlavePortParameters] => TLSlavePortParameters
  )(implicit override val valName: ValName)
      extends NexusNode(TLSlaveOnlyImp)(dFn, uFn)
