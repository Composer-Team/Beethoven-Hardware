...
class AcceleratorSystem(val nCores: Int, core_offset: Int)(implicit
    p: Parameters,
    val systemParams: AcceleratorSystemConfig,
    val on_deviceID: Int
) extends LazyModule {
...
//        println(s"Read should be [${blockBytes}, ${blockBytes * platform.prefetchSourceMultiplicity}]")
              ...
              TLMasterPortParameters.v1(
                clients = List(
                  TLMasterParameters.v1(
                    name =
                      s"ReadChannel_d${on_deviceID}_${systemParams.name}_core${core_id}_${para.name}$i",
                    supportsGet = TransferSizes(
                      blockBytes,
                      blockBytes * platform.prefetchSourceMultiplicity
                    ),
                    ...
                    sourceId = IdRange(
                      0,
                      param.maxInFlightTxs
                        .getOrElse(platform.defaultReadTXConcurrency)
                    )
                  )
                )
              )
            ...
            List(
              TLMasterPortParameters.v1(
                List(
                  TLMasterParameters.v1(
                    name =
                      s"WriteChannel_${systemParams.name}_core${core_id}_${para.name}$i",
                    sourceId = IdRange(
                      0,
                      para.maxInFlightTxs
                        .getOrElse(platform.defaultWriteTXConcurrency)
                    ),
  ...
  writers.foreach(
    _.foreach(
      _._2.foreach(
        _.portParams.foreach(a =>
          a.allSupportGet.max == 0 && a.allSupportPutFull.max > 0
        )
      )
    )
  )
  ...
  val scratch_mod = (0 until nCores).map(coreIdx =>
    memParams
      ...
      .map { param =>
        lazy val mod = LazyModuleWithFloorplan(
          ...
          name =
            s"${systemParams.name}_d${on_deviceID}_c${coreIdx}_${param.name}"
        )
  ...
  val readerNodes = (0 until nCores).map { coreIdx =>
    val extended_readers = (readers(coreIdx).map(_._2) :+
      scratch_mod(coreIdx).map(_._2.mem_reader).filter(_.isDefined).map(_.get))
      .flatMap { a =>
        extend_eles_via_protocol_node[TLNode](
          a,
          make_tl_buffer,
          tl_assign,
          platform.interCoreMemReductionLatency
        )
      }
...
//    println("N readers: " + extended_readers.length)
      ...
      xbar_tree_reduce_sources[TLNode](
        extended_readers.map { er =>
          val checkProt = TLSupportChecker(
            (a: TLEdgeIn) =>
              a.master.allSupportPutFull.max == 0 && a.master.allSupportGet.max > 0,
            "Protocol Exclusive: AS_rn_pre"
          )
          checkProt := er
          checkProt
        },
  ...
  val writerNodes = (0 until nCores).map { coreIdx =>
    val extended_writers = (writers(coreIdx).map(_._2) :+
      scratch_mod(coreIdx).map(_._2.mem_writer).filter(_.isDefined).map(_.get))
      .flatMap { a =>
        extend_eles_via_protocol_node[TLNode](
          a,
          make_tl_buffer,
  ...
  val (intraCoreMemSlaveIOs, intraCoreMemSlaveNode) = {
      ...
      memParams.filter(_.isInstanceOf[IntraCoreMemoryPortInConfig]).map {
        case mp: IntraCoreMemoryPortInConfig =>
          (0 until mp.nChannels) map { channel_id =>
            val ics_mod = LazyModule(
              new IntraCoreScratchpad(
    ...
    // group by config
    val squish = only_tl_components
      .groupBy(_._1)
      .map { case (para, para_groups) =>
        // group by core
        val core_squish: Seq[(TLSlaveNode, Int)] = para_groups
          .map(a => (a._2, a._3))
          .groupBy(_._2._1)
          .map { case (core, core_group) =>
            ...
            core_group.groupBy(_._2._2).map { case (_, channel_group) =>
              ...
              val sunk = xbar_tree_reduce_sinks[TLSlaveNode](
                channel_group.map(_._1),
                4,
                1,
                reduce_lambda,
                () => TLSlaveBuffer(),
        ...
        val sunk_cores = xbar_tree_reduce_sinks[TLSlaveNode](
          core_squish.map(_._1),
          2,
          1,
          reduce_lambda,
          () => TLSlaveBuffer(),
        ...
        sunk_cores(0) := TLSlaveBuffer() := tl2slave := TLWidthWidget(
          platform.interCoreMemBusWidthBytes
        ) := tl_head
        (para, tl_head)
      }
      .toList
  ...
  val intraCoreMemMasters =
    memParams.filter(_.isInstanceOf[IntraCoreMemoryPortOutConfig]).map {
      case mp: IntraCoreMemoryPortOutConfig =>
        val (otherSystemParams, otherSPParams) =
          try {
            val otherS = p(AcceleratorSystems).filter(_.name == mp.toSystem)(0)
            val otherSP =
              otherS.memoryChannelConfig.filter(_.name == mp.toMemoryPort)(0)
            (otherS, otherSP.asInstanceOf[IntraCoreMemoryPortInConfig])
          } catch {
            case a: Exception =>
              System.err.println(
                "Could not properly find system and memory port given by outward memory port params."
              )
              throw a
          }
        val memMasters = Seq.fill(otherSystemParams.nCores)(
          TLClientNode(
            Seq.fill(mp.nChannels)(
              TLMasterPortParameters.v1(
                clients = Seq(
                  TLMasterParameters.v1(
                    ...
                    supportsProbe = TransferSizes(
                      Math.max(
                        otherSPParams.dataWidthBits.intValue() / 8,
                        platform.interCoreMemBusWidthBytes
                      )
                    ),
                    supportsPutFull = TransferSizes(
        ...
        val masterShrunk =
          memMasters.map(TLWidthWidget(otherSPParams.dataWidthBits / 8) := _)
        (mp, memMasters, masterShrunk)
    }

  val Seq(r_nodes, w_nodes) = Seq((readerNodes, "r"), (writerNodes, "w")).map {
    case (nodes, ty) =>
