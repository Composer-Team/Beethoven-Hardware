...
import beethoven.Protocol.tilelink.{
  TLRWFilter,
  TLSourceShrinkerDynamicBlocking,
  TLSupportChecker,
  TLToAXI4SRW
}
...
object BeethovenTop {
  ...
  /** Get the address mask given the desired address space size (per DIMM) in
    ...
    * @param baseTotal
    *   mask for the channel bits - address bits are masked out for this
    * @param idx
    *   DO NOT DEFINE - recursive parameter
    * @param acc
    *   DO NOT DEFINE - recursive parameter
    * @return
    */
  ...
  def getAddressMask(
    ...
    if (addrBits == 0) acc
    else if (((baseTotal >> idx) & 1) != 0)
      getAddressMask(addrBits, baseTotal, idx + 1, acc)
    else
      getAddressMask(addrBits - 1, baseTotal, idx + 1, acc | (BigInt(1) << idx))
  }
...
class BeethovenTop(implicit p: Parameters) extends LazyModule {
  ...
  val (frontDMA_r, frontDMA_w) = if (frontDMA_joined.isDefined) {
    val maxTransfer = TransferSizes(
      platform.extMem.master.beatBytes,
      platform.extMem.master.beatBytes * (1 << AXI4Compat.lenWidth)
    )
    val splitter = LazyModule(
      new TLRWFilter(
        TLSlavePortParameters.v1(
          managers = Seq(
            TLSlaveParameters.v1(
              address = (0 until platform.memoryNChannels)
                .map(BeethovenTop.getAddressSet(_)),
    ...
    val indexer = LazyModule(
      new TLSourceShrinkerDynamicBlocking(1 << platform.extMem.master.idBits)
    )
  ...
  locally {
    val r_map = devices.flatMap { d =>
      ...
      val off_chip =
        if (
          frontDMA_r.isDefined && platform.physicalInterfaces
    ...
    val w_map = devices.flatMap { d =>
      ...
      val off_chip =
        if (
          frontDMA_w.isDefined && platform.physicalInterfaces
            .find(_.isInstanceOf[PhysicalHostInterface])
            .get
            .locationDeviceID == d.deviceId
        ) {
    ...
    val mem_sinks = platform.physicalInterfaces
      .filter(_.isInstanceOf[PhysicalMemoryInterface])
      .map(_.locationDeviceID)
      .distinct
        ...
        case pmi: PhysicalMemoryInterface =>
          ...
          if (r_exist || w_exist) {
            ...
            Seq((r_commits, "r"), (w_commits, "w"))
              ...
              .foreach { case (commit_set, ty) =>
                val xbar_s = xbar_tree_reduce_sources(
                  commit_set(pmi.locationDeviceID),
                ...
                def check(id: String): TLIdentityNode = ty match {
                  case "r" =>
                    TLSupportChecker.readCheck(
                      f"Protocol Exclusive: check_r${id}"
                    )
                  case "w" =>
                    TLSupportChecker.writeCheck(
                      f"Protocol Exclusive: check_w${id}"
                    )
                }

                sTLToAXI := check("post") := TLSourceShrinkerDynamicBlocking(
  ...
  locally {
    ...
    val host_intf: PhysicalHostInterface = platform.physicalInterfaces
      .filter(_.isInstanceOf[PhysicalHostInterface])(0)
      .asInstanceOf[PhysicalHostInterface]
    val relevant_devices: Set[Int] =
      Set.from(devices.map(_.deviceId)) + host_intf.locationDeviceID
    val emitters_per_device = relevant_devices.toList.flatMap { deviceID: Int =>
      val basis: List[(RoccNode, Int)] =
        (if (deviceID == host_intf.locationDeviceID)
           List((rocc_front, deviceID))
         else List())
      val extras = devices
        .filter(a => a.deviceId == deviceID)
        .map(sd => sd.source_rocc.map(a => (a, sd.deviceId)))
        .flatten
      basis ++ extras
    }
    ...
    val devices_with_sinks = devices.map { sd => sd.deviceId }.filter { sd =>
      p(AcceleratorSystems)
        .map(as => slr2ncores(sd, as.nCores)._1 > 0)
        .fold(false)(_ || _)
    }
    ...
    devices.filter(_.host_rocc.isDefined).foreach { sd =>
      ...
      if (sources.nonEmpty) {
        val source_unit = xbar_tree_reduce_sources(
          sources,
          platform.xbarMaxDegree,
          1,
          make_rocc_xbar,
          make_rocc_buffer,
          (a: Seq[RoccNode], b: RoccNode) => rocc_assign(a, b)(p)
        )(p)(0)
        sink := source_unit
      }
    }
