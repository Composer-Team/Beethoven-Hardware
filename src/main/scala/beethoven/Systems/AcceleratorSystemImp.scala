...
class AcceleratorSystemImp(val outer: AcceleratorSystem)(implicit p: Parameters)
  ...
  val cores = Seq.tabulate(outer.nCores) { core_idx: Int =>
    val altered = p.alterPartial { case OuterKey =>
      outer
    }
    val impl = Module(outer.systemParams.moduleConstructor match {
      case mb: ModuleBuilder => mb.constructor(altered)
      case bbc: BlackboxBuilderCustom =>
        new AcceleratorBlackBoxCore(bbc)(altered, outer.systemParams)
//      case bbv: BlackboxBuilderRocc => new AcceleratorBlackBoxCore(bbv)(altered, outer.systemParams)
  ...
  /* handle all memory */
  cores.zipWithIndex.zip(outer.readers) foreach {
    case ((core, coreIdx), readSet) =>
      readSet.foreach { case (nodeParams, nodes) =>
        val (cParams, clients) = core.read_ios(nodeParams.name)
        (nodes zip clients).zipWithIndex foreach {
          case ((node, client), channel_idx) =>
            val readerModule = ModuleWithFloorplan(
              new SequentialReader(
                client._2.data.bits.getWidth,
                node.out(0)._1,
                node.out(0)._2,
  ...
  cores.zipWithIndex.zip(outer.writers) foreach {
    case ((core, coreIdx), writeSet) =>
      writeSet foreach { case (nodeName, nodes) =>
        val (cParams, clients) = core.write_ios(nodeName.name)
        (nodes zip clients).zipWithIndex foreach {
          case ((node, client), channel_idx) =>
            val writerModule = ModuleWithFloorplan(
  ...
  cores.zip(outer.intraCoreMemSlaveIOs) foreach { case (core, sps) =>
    sps.flatten.foreach { case (config, spad, _, (core_idx, channel_idx)) =>
      core
        .intra_mem_ins(config.name)(channel_idx)
        .zip(spad.module.IOs)
        .foreach { case (a, b) => a <> b }
    }
  }

  cores.zip(outer.intraCoreMemMasters) foreach {
    case (core, (mp, nodeseq, _)) =>
