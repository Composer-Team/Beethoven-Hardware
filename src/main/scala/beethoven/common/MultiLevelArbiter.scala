...
class MultiLevelArbiter[T <: Data](gen: T, nIn: Int)(implicit p: Parameters)
    extends Module {
  ...
  require(
    isPow2(width),
    "Using this arbiter implementation requires the max XBar degree to be a power of 2"
  )
  ...
  if (nIn == 1) {
    ...
    val in_conv = io.in.zipWithIndex.map { case (dat, idx) =>
      dat.map { d =>
        val wire = Wire(new ArbIOQ); wire.chosen := idx.U; wire.data := d; wire
      }
    }
    val finalIO = recursivelyGroup(in_conv)
    io.out.valid := finalIO.valid
    require(
      finalIO.bits.chosen.getWidth == io.chosen.getWidth,
      s"Require same widths. Found ${finalIO.bits.chosen.getWidth} and ${io.chosen.getWidth}"
    )
